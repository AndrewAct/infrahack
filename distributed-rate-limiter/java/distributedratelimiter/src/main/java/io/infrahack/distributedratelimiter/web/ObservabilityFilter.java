 ...
 * {@code X-Request-Id} or a fresh one), echoes it in the response, times the request, updates the
 * metrics counters, and writes one summary log line. A filter rather than an interceptor so the
 * whole request is a single try/finally - no state split across callbacks.
 */
public final class ObservabilityFilter extends OncePerRequestFilter {
    ...
    // Static paths only; anything else becomes "other" so scanners probing random URLs cannot
    // mint unbounded label values (metric cardinality is a resource too).
    private static final Set<String> KNOWN_ROUTES = Set.of(
            "/v1/rate-limit/check", "/demo/ping", "/health", "/health/ready", "/metrics");
